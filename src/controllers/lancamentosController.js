...
async function criar(req, res) {
  try {
    const { clienteId } = req.params;
    const { tipo, valor, data, categoria, subcategoria, descricao, pagamento, status, quantidade, is_cmv, grupo_id, valor_recebido, origem, obs, valor_upgrade, qtd_upgrade, banco } = req.body;
    const valorNum = parseFloat(valor);
    if (!tipo || !data || isNaN(valorNum) || valorNum < 0) return res.status(400).json({ erro: 'Tipo, valor e data são obrigatórios' });
    const result = await pool.query(
      `INSERT INTO lancamentos
        (cliente_id, tipo, valor, data, categoria, subcategoria, descricao, pagamento, status, quantidade, is_cmv, grupo_id, valor_recebido, origem, obs, valor_upgrade, qtd_upgrade, banco)
...
async function editar(req, res) {
  try {
    ...
    const { tipo, valor, data, categoria, subcategoria, descricao, pagamento, status, quantidade, obs, valor_recebido, grupo_id, valor_upgrade, qtd_upgrade, banco } = req.body;
    const valorNum = parseFloat(valor);
    ...
    const result = await pool.query(
      `UPDATE lancamentos SET tipo=$1, valor=$2, data=$3, categoria=$4, subcategoria=$5,
       ...
       valor_recebido=$11, grupo_id=$12, valor_upgrade=$13, qtd_upgrade=$14, banco=$15
       WHERE id=$16 AND cliente_id=$17 RETURNING *`,
