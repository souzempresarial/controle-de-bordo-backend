  ...
  try {
    ...
    const { rows } = await pool.query(`
      SELECT
        c.id,
        c.nome,
        -- Faturamento: entradas usando valor_recebido quando disponível (igual ao DRE)
        COALESCE(SUM(CASE WHEN l.tipo = 'Entrada' THEN COALESCE(l.valor_recebido, l.valor) ELSE 0 END), 0) AS faturamento,
        ...
        -- Caixa bruto: todas entradas menos todas saídas
        COALESCE(SUM(CASE
          WHEN l.tipo = 'Entrada' THEN COALESCE(l.valor_recebido, l.valor)
    ...
    const resultado = rows.map(r => {
      ...
      // lucro = fat - cmv - sga - naoOp  (igual ao lucroLiq do DRE)
      const lucro = fat - cmv - sga - naoOp;
      return {
        id: r.id, nome: r.nome,
        faturamento:   fat,
        saidas:        cmv + sga + naoOp,
        cmv,
        sga,
        naoOp,
        caixa,
